BusApp.routes = (()=>{
  ...
  function renderList(list){const el=document.getElementById('dropdownBody');el.innerHTML=list.length?list.map(r=>`<li><button class="route-item ${S.selectedRouteId===r._realId?'active':''}" data-route-id="${r._realId}"><span class="route-item-icon">🚌</span><span class="route-item-label">Маршрут </span><span class="route-item-number">${label(r)}</span><span class="route-item-arrow">›</span></button></li>`).join(''):'<div class="dd-msg">Ничего не найдено</div>';}
  ...
  function renderTransit(v){const c=document.getElementById('transitResultsContainer'),l=document.getElementById('transitResultsList');window._transit=v;l.innerHTML=v.length?v.map((x,i)=>`<div class="transit-card" data-transit-index="${i}"><div class="transit-header"><div class="transit-badges">${x.routeNames.map(n=>`<span class="badge-route">Маршрут ${n}</span>`).join(' ➔ ')}<span class="badge-transfer ${x.transfers?'trans-1':''}">${x.transfers?'С пересадкой':'Прямой'}</span></div><span class="transit-time">~${x.estimatedTime} мин</span></div><div class="transit-details">🚶 До остановки: ${x.walkDist1} м (~${x.walkTime1} мин)</div>${x.transfers?`<div class="transit-details">🔁 Пересадка: ${x.transferStop.BusStopNameKA||x.transferStop.BusStopNameGeoGps||'остановка'}</div>`:''}<div class="transit-details">🚶 После остановки: ${x.walkDist2} м (~${x.walkTime2} мин)</div></div>`).join(''):'<div class="dd-msg">Маршруты не найдены</div>';c.style.display='block';}
  async function walkingLine(a,b){
    const straight=[[a.lon,a.lat],[b.lon,b.lat]];
    try{
      const url=`https://routing.openstreetmap.de/routed-foot/route/v1/driving/${a.lon},${a.lat};${b.lon},${b.lat}?overview=full&geometries=geojson&steps=false`;
      const r=await fetch(url); if(!r.ok) throw new Error('walking route');
      const j=await r.json(); const coords=j.routes?.[0]?.geometry?.coordinates;
      return Array.isArray(coords)&&coords.length>1?coords:straight;
    }catch(e){ console.warn('Пешеходный маршрут по дорогам недоступен, используется прямая линия'); return straight; }
  }
  async function selectTransit(i){
    const v=window._transit?.[i];if(!v)return;
    S.selectedTransitVariant=v;
    select(v.routeIds[0],{fit:false});
    showSelectedRoute(v.routeIds);
    const jobs=[];
    if(S.fromCoords)jobs.push(walkingLine(S.fromCoords,{lat:+v.startStop.BusStopLatitude,lon:+v.startStop.BusStopLongitude}));
    if(S.toCoords)jobs.push(walkingLine({lat:+v.endStop.BusStopLatitude,lon:+v.endStop.BusStopLongitude},S.toCoords));
    BusApp.mapApi.setPedestrianLines(await Promise.all(jobs));
