BusApp.routes = (()=>{
  ...
  const t=k=>BusApp.i18n.t(k);
  const label=r=>{const lang=BusApp.i18n.getLanguage();if(lang==='ka')return r?.RouteNameKA||r?.RouteNameGeoGps||r?.RouteNameEN||r?.RouteIdGeoGps||t('route');if(lang==='en'||lang==='uk')return r?.RouteNameEN||r?.RouteNameGeoGps||r?.RouteNameKA||r?.RouteIdGeoGps||t('route');return r?.RouteNameGeoGps||r?.RouteNameEN||r?.RouteNameKA||r?.RouteIdGeoGps||t('route');};
  ...
  function select(id,{fit=true}={}){
    ...
    document.getElementById('activeRouteStatusContainer').style.display='block';document.getElementById('activeRouteNameDisplay').textContent=`${t('route')} ${S.selectedRouteName}`;document.getElementById('liveBadge').style.display='flex';document.getElementById('legendBox').style.display='block';
  ...
  function findTransit(){
    if(!S.fromCoords||!S.toCoords){alert(t('specifyPoints'));return;}if(!S.db?.busStops){alert(t('stopsLoading'));return;}
  ...
  function renderTransit(v){const c=document.getElementById('transitResultsContainer'),l=document.getElementById('transitResultsList');window._transit=v;l.innerHTML=v.length?v.map((x,i)=>`<div class="transit-card" data-transit-index="${i}"><div class="transit-header"><div class="transit-badges">${x.routeNames.map(n=>`<span class="badge-route">${t('route')} ${n}</span>`).join(' ➔ ')}<span class="badge-transfer ${x.transfers?'trans-1':''}">${x.transfers?t('transfer'):t('direct')}</span></div><span class="transit-time">~${x.estimatedTime} ${t('min')}</span></div><div class="transit-details"><img class="text-icon" src="assets/icons/walk.png" alt=""> ${t('walkToStop')}: ${x.walkDist1} ${t('meters')} (~${x.walkTime1} ${t('min')})</div>${x.transfers?`<div class="transit-details"><img class="text-icon" src="assets/icons/transfer.png" alt=""> ${t('transferAt')}: ${x.transferStop.BusStopNameKA||x.transferStop.BusStopNameGeoGps||t('stop')}</div>`:''}<div class="transit-details"><img class="text-icon" src="assets/icons/walk.png" alt=""> ${t('afterStop')}: ${x.walkDist2} ${t('meters')} (~${x.walkTime2} ${t('min')})</div></div>`).join(''):`<div class="dd-msg">${t('routesNotFound')}</div>`;c.style.display='block';}
  ...
  function clear({preserveGps=true}={}){
    ...
    document.getElementById('fromInput').value=preserveGps&&S.lastPosition?t('gpsPosition'):'';document.getElementById('toInput').value='';document.getElementById('transitResultsContainer').style.display='none';document.getElementById('transitResultsList').innerHTML='';
    document.getElementById('activeRouteStatusContainer').style.display='none';document.getElementById('liveBadge').style.display='none';document.getElementById('legendBox').style.display='none';
    renderList(S.allRoutes);
  }
  function rerender(){if(S.selectedRouteId){const selected=routeObj(S.selectedRouteId);S.selectedRouteName=label(selected);}renderList(S.allRoutes);if(window._transit)renderTransit(window._transit);if(S.selectedRouteId)document.getElementById('activeRouteNameDisplay').textContent=`${t('route')} ${S.selectedRouteName}`;}
  return {initDb,renderList,filter,select,findTransit,selectTransit,clearLiveRoute,clear,label,rerender};
})();
