BusApp.buses = (()=>{
  ...
  function update(data){
    ...
    data.forEach(b=>{
      ...
      if(!obj){
        const el=makeElement(b.Status);const marker=new maplibregl.Marker({element:el,anchor:'center',subpixelPositioning:true}).setLngLat([lon,lat]).setPopup(new maplibregl.Popup({offset:18}).setHTML(`<b>${b.Name||t('bus')}</b><br>${b.Status===1?`${t('direction')}: ${t('outbound')}`:`${t('direction')}: ${t('inbound')}`}`)).addTo(BusApp.mapApi.map);
        ...
        obj.marker.getPopup()?.setHTML(`<b>${b.Name||t('bus')}</b><br>${b.Status===1?`${t('direction')}: ${t('outbound')}`:`${t('direction')}: ${t('inbound')}`}`);
      }
    });
