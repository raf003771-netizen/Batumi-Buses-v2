BusApp.ui = (()=>{
  ...
  function bindMap(){
    ...
    map.on('click',e=>{
      if(S.pickMode){const {lng,lat}=e.lngLat;const mode=S.pickMode;if(mode==='to'||mode==='to-auto'){BusApp.places.setDestination(lng,lat,'Точка на карте');document.getElementById('toInput').value=`${lat.toFixed(4)}, ${lng.toFixed(4)}`;}else{BusApp.gps.setManualLocation(lng,lat);S.fromCoords={lat,lon:lng};document.getElementById('fromInput').value=`${lat.toFixed(4)}, ${lng.toFixed(4)}`;}S.pickMode=null;map.getCanvas().style.cursor='';if(mode!=='to-auto')document.getElementById('appSidebar').classList.add('open');return;}
