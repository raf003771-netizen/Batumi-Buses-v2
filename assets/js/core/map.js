BusApp.mapApi = (() => {
  ...
  function installLayers() {
    ...
    map.addLayer({id:'route-stops-layer', type:'circle', source:'route-stops', paint:{'circle-radius':6,'circle-color':'#fff','circle-stroke-color':'#11a37a','circle-stroke-width':2}});
    map.on('mouseenter','route-stops-layer',()=>{ map.getCanvas().style.cursor='pointer'; });
    map.on('mouseleave','route-stops-layer',()=>{ map.getCanvas().style.cursor=''; });
  ...
  function setStops(stops) {
    BusApp.state.currentStops = stops || [];
    setData('route-stops', {type:'FeatureCollection',features:(stops||[]).map((s,index)=>({type:'Feature',geometry:{type:'Point',coordinates:[Number(s.BusStopLongitude),Number(s.BusStopLatitude)]},properties:{stopIndex:index,stopId:String(s.BusStopId||s._id||index),stopName:s.BusStopNameKA||s.BusStopNameGeoGps||s.BusStopNameEN||'Остановка'}}))});
  }
  ...
  function clearLiveRouteVisuals(){ BusApp.state.currentStops=[]; BusApp.stopBoard?.close(); clearSource('route-line'); clearSource('route-stops'); }
