BusApp.mapApi = (() => {
  ...
  function setRouteLine(coords) {
    setData('route-line', coords?.length ? {type:'Feature',geometry:{type:'LineString',coordinates:coords.map(c=>[Number(c.lon),Number(c.lat)])},properties:{}} : emptyCollection());
  }
  ...
  function clearRouteVisuals(){ clearSource('route-line');clearSource('pedestrian-lines');clearSource('route-stops'); }
  return {init,get map(){return map;},setRouteLine,setPedestrianLines,setStops,setAccuracy,smartCenter,fitCoordinates,clearRouteVisuals,clearSource};
})();
